...
use Duxbo\LaravelAuth\Http\Controllers\Web\Admin\PermissionController;
use Duxbo\LaravelAuth\Http\Controllers\Web\Admin\RoleController;
use Duxbo\LaravelAuth\Http\Controllers\Web\Admin\UserController;
...
// RolePolicy/PermissionPolicy/UserPolicy — a super-admin role (Gate::before
// in the ServiceProvider) bypasses all of it. {user} is bound explicitly
// since the User model is only known at runtime via config.
Route::model('user', config('laravel-auth.user_model'));

$userModelClass = config('laravel-auth.user_model');

Route::middleware('auth')->prefix('admin')->name('admin.')->group(function () use ($userModelClass) {
